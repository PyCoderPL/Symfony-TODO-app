...
class ConnectionConfig
{
    ...
    public function __construct(array $value = [])
    {
        ...
        if (array_key_exists('slaves', $value)) {
            $this->_usedProperties['slaves'] = true;
            $this->slaves = array_map(
                function ($v) {
                    return \is_array($v) ? new \Symfony\Config\Doctrine\Dbal\ConnectionConfig\SlaveConfig($v) : $v; 
                }, $value['slaves']
            );
            unset($value['slaves']);
        }
        ...
        if (array_key_exists('replicas', $value)) {
            ...
            $this->replicas = array_map(
                function ($v) {
                    return \is_array($v) ? new \Symfony\Config\Doctrine\Dbal\ConnectionConfig\ReplicaConfig($v) : $v; 
                }, $value['replicas']
            );
    ...
    public function toArray(): array
    {
        ...
        if (isset($this->_usedProperties['slaves'])) {
            $output['slaves'] = array_map(
                function ($v) {
                    return $v instanceof \Symfony\Config\Doctrine\Dbal\ConnectionConfig\SlaveConfig ? $v->toArray() : $v; 
                }, $this->slaves
            );
        }
        if (isset($this->_usedProperties['replicas'])) {
            $output['replicas'] = array_map(
                function ($v) {
                    return $v instanceof \Symfony\Config\Doctrine\Dbal\ConnectionConfig\ReplicaConfig ? $v->toArray() : $v; 
                }, $this->replicas
            );
        }
