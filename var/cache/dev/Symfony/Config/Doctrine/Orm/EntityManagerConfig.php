...
class EntityManagerConfig
{
    ...
    public function __construct(array $value = [])
    {
        ...
        if (array_key_exists('mappings', $value)) {
            $this->_usedProperties['mappings'] = true;
            $this->mappings = array_map(
                function ($v) {
                    return \is_array($v) ? new \Symfony\Config\Doctrine\Orm\EntityManagerConfig\MappingConfig($v) : $v; 
                }, $value['mappings']
            );
        ...
        if (array_key_exists('filters', $value)) {
            ...
            $this->filters = array_map(
                function ($v) {
                    return \is_array($v) ? new \Symfony\Config\Doctrine\Orm\EntityManagerConfig\FilterConfig($v) : $v; 
                }, $value['filters']
            );
            unset($value['filters']);
        }
    ...
    public function toArray(): array
    {
        ...
        if (isset($this->_usedProperties['mappings'])) {
            $output['mappings'] = array_map(
                function ($v) {
                    return $v instanceof \Symfony\Config\Doctrine\Orm\EntityManagerConfig\MappingConfig ? $v->toArray() : $v; 
                }, $this->mappings
            );
        }
        ...
        if (isset($this->_usedProperties['filters'])) {
            $output['filters'] = array_map(
                function ($v) {
                    return $v instanceof \Symfony\Config\Doctrine\Orm\EntityManagerConfig\FilterConfig ? $v->toArray() : $v; 
                }, $this->filters
            );
        }
