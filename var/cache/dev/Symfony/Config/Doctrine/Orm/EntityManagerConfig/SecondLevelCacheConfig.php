    ...
    public function __construct(array $value = [])
    {
        ...
        if (array_key_exists('regions', $value)) {
            ...
            $this->regions = array_map(
                function ($v) {
                    return new \Symfony\Config\Doctrine\Orm\EntityManagerConfig\SecondLevelCache\RegionConfig($v); 
                }, $value['regions']
            );
            unset($value['regions']);
        }
        ...
        if (array_key_exists('loggers', $value)) {
            $this->_usedProperties['loggers'] = true;
            $this->loggers = array_map(
                function ($v) {
                    return new \Symfony\Config\Doctrine\Orm\EntityManagerConfig\SecondLevelCache\LoggerConfig($v); 
                }, $value['loggers']
            );
            unset($value['loggers']);
        }
