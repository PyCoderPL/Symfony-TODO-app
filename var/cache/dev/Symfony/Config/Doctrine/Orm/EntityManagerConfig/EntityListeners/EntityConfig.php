...
class EntityConfig
{
    ...
    public function __construct(array $value = [])
    {
        if (array_key_exists('listeners', $value)) {
            $this->_usedProperties['listeners'] = true;
            $this->listeners = array_map(
                function ($v) {
                    return new \Symfony\Config\Doctrine\Orm\EntityManagerConfig\EntityListeners\EntityConfig\ListenerConfig($v); 
                }, $value['listeners']
            );
