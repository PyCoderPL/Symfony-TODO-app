...
class DbalConfig
{
    ...
    public function __construct(array $value = [])
    {
        ...
        if (array_key_exists('types', $value)) {
            ...
            $this->types = array_map(
                function ($v) {
                    return \is_array($v) ? new \Symfony\Config\Doctrine\Dbal\TypeConfig($v) : $v; 
                }, $value['types']
            );
            unset($value['types']);
        }
        ...
        if (array_key_exists('connections', $value)) {
            ...
            $this->connections = array_map(
                function ($v) {
                    return \is_array($v) ? new \Symfony\Config\Doctrine\Dbal\ConnectionConfig($v) : $v; 
                }, $value['connections']
            );
    ...
    public function toArray(): array
    {
        ...
        if (isset($this->_usedProperties['types'])) {
            $output['types'] = array_map(
                function ($v) {
                    return $v instanceof \Symfony\Config\Doctrine\Dbal\TypeConfig ? $v->toArray() : $v; 
                }, $this->types
            );
        }
        ...
        if (isset($this->_usedProperties['connections'])) {
            $output['connections'] = array_map(
                function ($v) {
                    return $v instanceof \Symfony\Config\Doctrine\Dbal\ConnectionConfig ? $v->toArray() : $v; 
                }, $this->connections
            );
        }
