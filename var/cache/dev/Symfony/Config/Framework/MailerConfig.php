...
class MailerConfig
{
    ...
    public function __construct(array $value = [])
    {
        ...
        if (array_key_exists('headers', $value)) {
            ...
            $this->headers = array_map(
                function ($v) {
                    return \is_array($v) ? new \Symfony\Config\Framework\Mailer\HeaderConfig($v) : $v; 
                }, $value['headers']
            );
            unset($value['headers']);
        }
    ...
    public function toArray(): array
    {
        ...
        if (isset($this->_usedProperties['headers'])) {
            $output['headers'] = array_map(
                function ($v) {
                    return $v instanceof \Symfony\Config\Framework\Mailer\HeaderConfig ? $v->toArray() : $v; 
                }, $this->headers
            );
        }
