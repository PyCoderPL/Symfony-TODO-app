...
class CacheConfig
{
    ...
    public function __construct(array $value = [])
    {
        ...
        if (array_key_exists('pools', $value)) {
            $this->_usedProperties['pools'] = true;
            $this->pools = array_map(
                function ($v) {
                    return \is_array($v) ? new \Symfony\Config\Framework\Cache\PoolConfig($v) : $v; 
                }, $value['pools']
            );
            unset($value['pools']);
        }
    ...
    public function toArray(): array
    {
        ...
        if (isset($this->_usedProperties['pools'])) {
            $output['pools'] = array_map(
                function ($v) {
                    return $v instanceof \Symfony\Config\Framework\Cache\PoolConfig ? $v->toArray() : $v; 
                }, $this->pools
            );
        }
