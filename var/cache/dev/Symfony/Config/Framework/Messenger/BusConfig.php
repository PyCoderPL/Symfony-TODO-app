...
class BusConfig
{
    ...
    public function __construct(array $value = [])
    {
        ...
        if (array_key_exists('middleware', $value)) {
            ...
            $this->middleware = array_map(
                function ($v) {
                    return \is_array($v) ? new \Symfony\Config\Framework\Messenger\BusConfig\MiddlewareConfig($v) : $v; 
                }, $value['middleware']
            );
    ...
    public function toArray(): array
    {
        ...
        if (isset($this->_usedProperties['middleware'])) {
            $output['middleware'] = array_map(
                function ($v) {
                    return $v instanceof \Symfony\Config\Framework\Messenger\BusConfig\MiddlewareConfig ? $v->toArray() : $v; 
                }, $this->middleware
            );
        }
