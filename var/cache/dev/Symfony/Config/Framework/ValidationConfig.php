...
class ValidationConfig
{
    ...
    public function __construct(array $value = [])
    {
        ...
        if (array_key_exists('auto_mapping', $value)) {
            $this->_usedProperties['autoMapping'] = true;
            $this->autoMapping = array_map(
                function ($v) {
                    return \is_array($v) ? new \Symfony\Config\Framework\Validation\AutoMappingConfig($v) : $v; 
                }, $value['auto_mapping']
            );
    ...
    public function toArray(): array
    {
        ...
        if (isset($this->_usedProperties['autoMapping'])) {
            $output['auto_mapping'] = array_map(
                function ($v) {
                    return $v instanceof \Symfony\Config\Framework\Validation\AutoMappingConfig ? $v->toArray() : $v; 
                }, $this->autoMapping
            );
        }
