    ...
    public function __construct(array $value = [])
    {
        ...
        if (array_key_exists('http_codes', $value)) {
            ...
            $this->httpCodes = array_map(
                function ($v) {
                    return \is_array($v) ? new \Symfony\Config\Framework\HttpClient\DefaultOptions\RetryFailed\HttpCodeConfig($v) : $v; 
                }, $value['http_codes']
            );
    ...
    public function toArray(): array
    {
        ...
        if (isset($this->_usedProperties['httpCodes'])) {
            $output['http_codes'] = array_map(
                function ($v) {
                    return $v instanceof \Symfony\Config\Framework\HttpClient\DefaultOptions\RetryFailed\HttpCodeConfig ? $v->toArray() : $v; 
                }, $this->httpCodes
            );
        }
