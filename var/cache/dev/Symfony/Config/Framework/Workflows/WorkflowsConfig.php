...
class WorkflowsConfig
{
    ...
    public function __construct(array $value = [])
    {
        ...
        if (array_key_exists('places', $value)) {
            ...
            $this->places = array_map(
                function ($v) {
                    return \is_array($v) ? new \Symfony\Config\Framework\Workflows\WorkflowsConfig\PlaceConfig($v) : $v; 
                }, $value['places']
            );
            unset($value['places']);
        }
        ...
        if (array_key_exists('transitions', $value)) {
            ...
            $this->transitions = array_map(
                function ($v) {
                    return \is_array($v) ? new \Symfony\Config\Framework\Workflows\WorkflowsConfig\TransitionConfig($v) : $v; 
                }, $value['transitions']
            );
    ...
    public function toArray(): array
    {
        ...
        if (isset($this->_usedProperties['places'])) {
            $output['places'] = array_map(
                function ($v) {
                    return $v instanceof \Symfony\Config\Framework\Workflows\WorkflowsConfig\PlaceConfig ? $v->toArray() : $v; 
                }, $this->places
            );
        }
        if (isset($this->_usedProperties['transitions'])) {
            $output['transitions'] = array_map(
                function ($v) {
                    return $v instanceof \Symfony\Config\Framework\Workflows\WorkflowsConfig\TransitionConfig ? $v->toArray() : $v; 
                }, $this->transitions
            );
        }
