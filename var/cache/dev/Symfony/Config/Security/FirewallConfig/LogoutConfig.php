...
class LogoutConfig
{
    ...
    public function __construct(array $value = [])
    {
        ...
        if (array_key_exists('delete_cookies', $value)) {
            $this->_usedProperties['deleteCookies'] = true;
            $this->deleteCookies = array_map(
                function ($v) {
                    return \is_array($v) ? new \Symfony\Config\Security\FirewallConfig\Logout\DeleteCookieConfig($v) : $v; 
                }, $value['delete_cookies']
            );
    ...
    public function toArray(): array
    {
        ...
        if (isset($this->_usedProperties['deleteCookies'])) {
            $output['delete_cookies'] = array_map(
                function ($v) {
                    return $v instanceof \Symfony\Config\Security\FirewallConfig\Logout\DeleteCookieConfig ? $v->toArray() : $v; 
                }, $this->deleteCookies
            );
        }
