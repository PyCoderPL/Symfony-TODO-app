...
class MappingException extends ORMException
{
    ...
    public static function duplicateDiscriminatorEntry($className, array $entries, array $map)
    {
        return new self(
            ...
            'The entries of the current map are: @DiscriminatorMap({' . implode(
                ', ', array_map(
                    static function ($a, $b) {
    ...
    {
        return new self(
            "It is not possible to map entity '" . $className . "' with a composite primary key " .
    ...
    {
        return new self(
            'Orphan removal is only allowed on one-to-one and one-to-many ' .
