        ...
        $tokenHandlerNodeBuilder = $builder
            ...
            ->then(
                ...
                ->beforeNormalization()
                    ->ifTrue(
                        static function ($v) {
                            return \is_array($v) && 1 < \count($v); 
                        }
                    )
                    ->then(
                        static function () {
                            throw new InvalidConfigurationException('You cannot configure multiple token handlers.'); 
                        }
                    )
                ->end()
                ...
                ->beforeNormalization()
                    ...
                    ->then(
                        static function () {
                            throw new InvalidConfigurationException('You must set a token handler.'); 
                        }
                    )
                ->end()
